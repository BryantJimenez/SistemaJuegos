...
$(document).ready(function() {
    ...
    $('input[name="name"], input[name="lastname"]').keypress(function() {
    ...
    //multiselect
    if ($('.multiselect, .multiselectGamers').length) {
        $('.multiselect, .multiselectGamers').multiselect({
...
function deleteTourament(slug) {
    $("#deleteTourtament").modal();
    $('#formDeleteTourtament').attr('action', '/torneos/' + slug);
}
...
// Función para agregar jugadores al torneo
function addGamers(slug) {
    $.ajax({
        url: '/torneos/'+slug+'/agregar',
        dataType: 'html',
    })
    .done(function(resultado) {
        $(".multiselectGamers option").remove();
        var obj=JSON.parse(resultado);
        if (obj[0].cupos>0) {
            for (var i=obj.length-1; i>=0; i--) {
                $('.multiselectGamers').append($('<option>', {
                    value: obj[i].slug,
                    text: obj[i].nombre_completo
                }));
            }
            $('.multiselectGamers').multiselect('destroy');
            $('.multiselectGamers').multiselect({
                buttonClass: 'form-control',
                buttonWidth: '100%',
                maxHeight: 200,
                includeSelectAllOption: false,
                ...
                onChange : function(option, checked) {
                    var selectedOptions = $(".multiselectGamers option:selected");

                    if (selectedOptions.length >= obj[0].cupos) {
                        var nonSelectedOptions = $('.multiselectGamers option').filter(function() {
                            return !$(this).is(':selected');
                        });
                        ...
                        $('.multiselectGamers option').each(function() {
                            var input = $('input[value="' + $(this).val() + '"]');
                            input.prop('disabled', false);
                            input.parent('li').addClass('disabled');
                        });
            ...
            $('#formAddGamers').attr('action', '/torneos/' + slug + '/agregar');
            $('#formAddGamersFull').css('display', 'none');
            $('#formAddGamers').css('display', 'block');
        } else {
            $('#formAddGamers').css('display', 'none');
            $('#formAddGamersFull').css('display', 'block');
        }
        $("#addGamers").modal();
    });
}
...
function addCouples(slug) {
    ...
    .done(function(resultado) {
        ...
        if (obj[0].cupos>0) {
            ...
            $('#formAddCouples').attr('action', '/parejas/' + slug + '/agregar');
