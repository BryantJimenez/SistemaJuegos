            ...
            if ($('#drop-area').length) {
                $('#drop-area').dmUploader({
                    ...
                    onBeforeUpload: function(){
                        ...
                        $("#respuesta").text('Subiendo Archivo...');
                    },
                    onUploadSuccess: function(id, data){
                        var obj=JSON.parse(data);
                        var text=$("#archivos").text();
                        var val=$("#nombreArchivos").val();
                        if (text=="") {
                            $("#archivos").text(obj.name);
                            $("#nombreArchivos").val(obj.nameFile);
                        } else {
                            $("#archivos").text(obj.name+', '+text);
                            $("#nombreArchivos").val(obj.nameFile+','+val);
                        }
                    ...
                    onFileSizeError: function(file){
                        $('button').attr('disabled', false);
                        $("#respuesta").text('El archivo \'' + file.name + '\' excede el tamaño máximo permitido.');
                    }
                });
            }

            if ($('#drop-area2').length) {
                var activoSlug=$('#slug').val();
                $('#drop-area2').dmUploader({
                    url: '/imagenes/productos/'+activoSlug,
                    maxFileSize: 3000000,
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    ...
                    onUploadSuccess: function(id, data){
                        ...
                        if (obj.status=='ok') {
                            $("#imagenes").append($('<div>', {
                                ...
                                'onclick': 'deleteImageProduct("foto'+id+'", "'+obj.nameFile+'", "'+obj.slug+'");'
...
$(document).ready(function() {
    ...
    //Value para exportar datos o no
    ...
    $('.btn-absolute-right').click(function(event) {
        var img=$(this).attr('photo');
        var activoSlug=$('#slug').val();
        var urlPhoto=event.currentTarget.attributes[3].value;
        urlPhoto=urlPhoto.split('/');
        $.ajax({
            url: '/imagenes/productos/'+activoSlug+'/'+urlPhoto[6]
        })
...
function showUser(slug) {
    ...
    .done(function(resultado) {
        ...
        $('#correoUsuario').text(obj.correo);
        $('#permisoUsuario').text(obj.permisos);
        $("#showUser").modal();
    });
};
...
function deleteImageProduct(img, nombre, slug){
    $.ajax({
        url: '/imagenes/productos/'+slug+'/'+nombre
    })
    .done(function(resultado) {
        var obj=JSON.parse(resultado);
        if (obj.status=='ok') {
